...
void main() {
    ...
    if (v_is_side > 0.5 && v_height_m >= 3.0) {
        ...
        float window_spacing = 250.0;
        float cell_u = fract((v_wall_uv.x - v_ed_flat) / window_spacing);
        ...
        if (win_mask > 0.01) {
            ...
            vec2 grid_id = floor(vec2((v_wall_uv.x - v_ed_flat) / window_spacing, v_wall_uv.y * num_floors));
